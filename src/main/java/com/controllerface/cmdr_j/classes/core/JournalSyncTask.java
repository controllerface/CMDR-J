...
public class JournalSyncTask implements Runnable
{
    ...
    public JournalSyncTask(GameState gameState)
    {
        ...
        while (go)
        {
            ...
            watchKey.pollEvents()
                    .forEach(watchEvent ->
                    {
                        ...
                        processEventFile(next, !next.getName().equals(currentJournalFile.get()));
                    });
            watchKey.reset();
        }
    }
    ...
    {
        File journalFolder = new File(JournalSyncTask.journalFolder);
            ...
            .ifPresentOrElse(handler ->
                {
                    ...
                    handler.process(gameState, eventData);
                },
                () ->
                {
                    if (debug)
